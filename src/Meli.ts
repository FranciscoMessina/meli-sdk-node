...
type HttpInstanceOptions = Omit<Options,
...
export class Meli {
    ...
    private parseReturn<T>(result: T | MeliApiError): MeliResponse<T> {
        if ( 'error' in result ) {
    ...
    async getQuestions<T = MeliQuestionsResponse>(
        ...
        if ( 'status' in options ) {
            searchParams.append( 'status', options.status as string || '' );
        }
    ...
    async askQuestion<T>(options: AskQuestionOptions): Promise<MeliResponse<T>> {
        if ( !options || !options.text || !options.itemId ) {
            throw new Error( 'Missing required options to ask a question.' )
        }
    ...
    async answerQuestion<T>(
        ...
        if ( !questionId || answer ) {
            throw new Error( 'Missing required options to answer a question.' )
        }
    ...
    /** Bloquea a un usuario de hacer preguntas en la cuenta actual (Access Token utilizado) */
    async blockUser<T>(userToBlockId: number): Promise<MeliResponse<T>> {
        if ( !userToBlockId ) {
            throw new Error( 'userToBlockId is required to block a user.' )
        }
    ...
    /** Obtiene todos los usuarios bloqueados de la cuenta actual (Access Token utilizado). */
    ...
    /** Desbloquea a un usuario para que pueda volver a realizar preguntas a la cuenta actual. */
    ...
    /** Busca items por sitio de pais (MLA, MLB, etc..) en la API publica de MELI */
    ...
    async getQuestionsResponseTime<T = MeliQuestionsResponseTime>(): Promise<MeliResponse<T>> {
        const { body } = await this.http.get<T>(
